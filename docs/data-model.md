...
### Dissect architecture cache

The first successful codebase dissection is written to `$PASEO_HOME/dissect/projects/{projectId}.json` and, once, to the local SpacetimeDB `codebase_architecture` table. Later daemon homes restore that architecture from SpacetimeDB when the JSON file is empty.

### Dissect knowledge

Explicit knowledge lives in `$PASEO_HOME/dissect/knowledge.json` and `$PASEO_HOME/dissect/identity.json`. SpacetimeDB is the durable store for that state: `user_profile`, `concept_knowledge`, `project_knowledge`, `explanation_preference`, `feedback_event`, and `project_identity`. Local JSON is the outage queue. See [Dissect SpacetimeDB](development.md#dissect-spacetimedb).

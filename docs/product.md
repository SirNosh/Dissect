...
Dissect is an on-demand comprehension layer for a local coding-agent environment. You run agents on your machine; Dissect explains the codebase and the changes those agents make, then lets you keep working from desktop, web, or the `dissect` CLI.

**Dissect has a lean, opinionated core built to be extended.** It should be easy to start using and leave room for people to build far beyond the default experience.

## Agents and comprehension

The central workflow is giving an agent a task, understanding what it is doing, providing direction, and reviewing the result. Dissect sits on that workflow: first-pass architecture and file explanations from local files, then Dissect Diff for the last completed agent turn.

This is the basis for product decisions. A familiar feature from another development tool earns its place by improving that experience.
...
With a supported coding agent installed and authenticated, you can open the desktop app and start working. The app manages its local daemon. To connect your phone through the optional encrypted relay, enable pairing and scan a QR code. You do not need to configure a VPN or understand the networking behind it.
...
Dissect should provide useful defaults while keeping these choices available. You should not have to adopt every part of the system to benefit from the parts you want.

See [architecture](architecture.md) for the system design.

## Freedom, ownership, and privacy

Dissect's flexibility depends on keeping control with the user:
...
- **Privacy:** No telemetry, tracking, or forced account. The relay is optional and end-to-end encrypted.
- **Open source:** Dissect is Apache-2.0 licensed. You can inspect it, modify it, and build on it.
...
Plugins are a central part of how Dissect gains capability.
...
Dissect's role is to provide interfaces that make those contributions possible across the daemon and connected clients. The [plugin documentation](plugins.md) describes what each released API supports.
...
Writing an implementation is one part of this process. Product judgment, design, QA, review, and learning from use determine how quickly a change can responsibly become part of Dissect.
...
Dissect is an independent project. Sustainable development, a coherent product, and the freedom to keep building are priorities. See [Contributing](../CONTRIBUTING.md) for how to participate.

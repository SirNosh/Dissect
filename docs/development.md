...
SpacetimeDB is the durable store for explicit Dissect knowledge, not repository contents. The local JSON files remain the outage fallback. Dissect Diff stays in the local JSON store. SpacetimeDB terminology is not shown in the product UI.

The module persists:

- a stable anonymous installation identity (`user_profile`);
- global concept familiarity (`concept_knowledge`);
- per-project component familiarity (`project_knowledge`);
- explanation preferences (`explanation_preference`);
- explicit `I know this` / `Explain more` events (`feedback_event`);
...
The daemon defaults to `http://127.0.0.1:3000` / `dissect`. Override with `DISSECT_SPACETIMEDB_URL` and `DISSECT_SPACETIMEDB_DB`, or set `DISSECT_SPACETIMEDB_DISABLED=1`.

The module lives in `spacetime/dissect`. Generated TypeScript bindings land in `spacetime/dissect/bindings`. Do not restart the main daemon on port 6767 after publishing.
...
Use `npm run cli` to run the in-repo CLI from source (`npx tsx packages/cli/src/index.ts`). The script wraps the CLI with `scripts/dev-home.sh`, so it automatically uses this checkout's `.dev/paseo-home` and dev daemon endpoint unless you pass an explicit override. After `npm run build:server`, `npx dissect` is the same CLI. The globally installed `dissect` binary on macOS may be a symlink into the installed Dissect desktop app, not this checkout — use `npm run cli` when you want to talk to the CLI you are editing.

Canonical automation uses `dissect project create/ls/rename/delete`, `dissect workspace create/ls/rename/archive`, `dissect heartbeat create/update/delete`, and the full `dissect schedule` group. MCP heartbeat automation is intentionally smaller: create and delete only. Detach remains an explicit user lifecycle action rather than an agent tool. `dissect run --new-workspace local|worktree` composes workspace creation with agent creation.

...
Local Android builds run on macOS (or Linux) and need the Android toolchain, pinned in `.mise.toml` (`java 21`, `android-sdk 21.0`). With [mise](https://mise.jdx.dev):

```bash
mise install        # java 21 + android-sdk 21.0 command-line tools
```

> **Pin a real `android-sdk` version, not `latest`.** The mise `android-sdk` plugin's `latest` resolved to the ancient `1.0` bundle, whose `sdkmanager` (3.6.0) predates the `emulator` package and fails with `Failed to find package emulator`. `21.0` ships a current `sdkmanager`. If you bump it, update `.mise.toml`.
...
This repository does not ship F-Droid store listing files. If you publish an F-Droid build, keep the listing with that build, not in this tree.

...
export async function createPaseoDaemon(
  ...
  const agentManager = new AgentManager({
    ...
    onAgentTurnBoundary: ({ cwd, phase }) => {
      const dissect = getDissectService(config.paseoHome, logger);
      if (phase === "start") {
        void dissect.recordAgentTurnStart(cwd);
      } else {
        void dissect.recordAgentTurnEnd(cwd);
      }
    },

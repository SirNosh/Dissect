...
import { DissectSession } from "./session/dissect/dissect-session.js";
import { getDissectService } from "../dissect/index.js";
...
export class Session {
  ...
  private readonly dissectSession: DissectSession;
  ...
  constructor(options: SessionOptions) {
    ...
    this.dissectSession = new DissectSession({
      host: { emit: (msg) => this.emit(msg) },
      service: getDissectService(paseoHome, this.sessionLogger),
      logger: this.sessionLogger,
    });
  ...
  private async dispatchInboundMessage(msg: SessionInboundMessage, source?: object): Promise<void> {
    ...
    await (promise ?? this.dispatchDissectMessage(msg) ?? this.dispatchMiscMessage(msg));
  }

  private dispatchDissectMessage(msg: SessionInboundMessage): Promise<void> | undefined {
    switch (msg.type) {
      case "dissect.state.get.request":
        return this.dissectSession.handleStateGet(msg);
      case "dissect.codebase.start.request":
        return this.dissectSession.handleCodebaseStart(msg);
      case "dissect.folder.get.request":
        return this.dissectSession.handleFolderGet(msg);
      case "dissect.file.get.request":
        return this.dissectSession.handleFileGet(msg);
      case "dissect.diff.start.request":
        return this.dissectSession.handleDiffStart(msg);
      case "dissect.diff.get_file.request":
        return this.dissectSession.handleDiffGetFile(msg);
      case "dissect.changes.check.request":
        return this.dissectSession.handleChangesCheck(msg);
      case "dissect.chat.ask.request":
        return this.dissectSession.handleChatAsk(msg);
      case "dissect.knowledge.signal.request":
        return this.dissectSession.handleKnowledgeSignal(msg);
      default:
        return undefined;
    }
  }

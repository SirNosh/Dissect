...
test("onAgentTurnBoundary is notified at live turn start and end", async () => {
  const workdir = mkdtempSync(join(tmpdir(), "agent-manager-turn-boundary-"));
  const storagePath = join(workdir, "agents");
  const storage = new AgentStorage(storagePath, logger);
  const onAgentTurnBoundary = vi.fn();
  ...
  await manager.runAgent(snapshot.id, { text: "do work" });

  expect(onAgentTurnBoundary.mock.calls).toEqual([
    [{ cwd: workdir, phase: "start" }],
    [{ cwd: workdir, phase: "end" }],
  ]);
});

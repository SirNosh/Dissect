...
export interface AgentManagerOptions {
  ...
  onAgentTurnBoundary?: (params: { cwd: string; phase: "start" | "end" }) => void;
...
export class AgentManager {
  ...
  private onAgentTurnBoundary?: (params: { cwd: string; phase: "start" | "end" }) => void;
  ...
  constructor(options: AgentManagerOptions) {
    ...
    this.onAgentTurnBoundary = options.onAgentTurnBoundary;
  ...
  private notifyAgentTurnBoundary(cwd: string, phase: "start" | "end"): void {
    try {
      this.onAgentTurnBoundary?.({ cwd, phase });
    } catch (error) {
      this.logger.warn({ err: error, cwd, phase }, "onAgentTurnBoundary failed");
    }
  }
  ...
  private openActiveTurn(
    ...
    if (notify) this.notifyAgentTurnBoundary(agent.cwd, "start");
  }
  ...
  private applyActiveTurnTerminal(
    ...
    this.notifyAgentTurnBoundary(agent.cwd, "end");
  ...
  private onStreamTurnStarted(params: {
    ...
    const { agent, eventTurnId, isForegroundEvent, flags, fromHistory } = params;
    this.logger.trace(
      {
        agentId: agent.id,
        provider: agent.provider,
    ...
    if (eventTurnId) {
      this.openActiveTurn(agent, eventTurnId, new Date(), !fromHistory);
    }
    agent.lifecycle = "running";
    this.emitState(agent);
  }

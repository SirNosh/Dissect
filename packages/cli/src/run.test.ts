...
describe("runCli", () => {
  ...
  it("defaults an empty CLI invocation to onboard", () => {
    expect(
      createCliParseArgv({
        argv: [],
        cwd: process.cwd(),
        nodeArgv: ["node", "dissect"],
      }),
    ).toEqual(["node", "dissect", "onboard"]);
  });
  ...
  it("routes explicit root relay flags to onboard", () => {
    ...
    ).toEqual(["node", "dissect", "onboard", "--relay"]);
    expect(
      createCliParseArgv({
        argv: ["--no-relay"],
        cwd: process.cwd(),
    ...
    ).toEqual(["node", "dissect", "onboard", "--no-relay"]);
  });
  ...
  it("preserves known CLI command argv", () => {
    ...
    ).toEqual(["node", "dissect", "daemon", "set-password"]);
  });
  ...
  it("preserves the hooks command argv", () => {
    ...
    ).toEqual(["node", "dissect", "hooks", "claude", "UserPromptSubmit"]);
  });

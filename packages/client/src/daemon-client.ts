...
export class DaemonClient {
  ...
  // ==========================================================================
  // Dissect comprehension layer
  // ==========================================================================

  async dissectStateGet(cwd: string, requestId?: string) {
    return this.sendNamespacedCorrelatedSessionRequest<"dissect.state.get.response">({
      requestId,
      message: { type: "dissect.state.get.request", cwd },
      timeout: 120_000,
    });
  }

  async dissectCodebaseStart(
    cwd: string,
    onProgress?: (
      progress: Extract<SessionOutboundMessage, { type: "dissect.progress" }>["payload"],
    ) => void,
    requestId?: string,
  ) {
    const resolvedRequestId = this.createRequestId(requestId);
    const unsubscribe = onProgress
      ? this.on("dissect.progress", (message) => {
          if (message.payload.requestId === resolvedRequestId) onProgress(message.payload);
        })
      : null;
    try {
      return await this.sendNamespacedCorrelatedSessionRequest<"dissect.codebase.start.response">({
        requestId: resolvedRequestId,
        message: { type: "dissect.codebase.start.request", cwd },
        timeout: 1_800_000,
      });
    } finally {
      unsubscribe?.();
  ...
  async dissectFolderGet(cwd: string, runId: string, path: string, requestId?: string) {
    return this.sendNamespacedCorrelatedSessionRequest<"dissect.folder.get.response">({
      requestId,
      message: { type: "dissect.folder.get.request", cwd, runId, path },
      timeout: 60_000,
    });
  }

  async dissectFileGet(cwd: string, path: string, requestId?: string) {
    return this.sendNamespacedCorrelatedSessionRequest<"dissect.file.get.response">({
      requestId,
      message: { type: "dissect.file.get.request", cwd, path },
      timeout: 300_000,
    });
  }

  async dissectDiffStart(
    cwd: string,
    onProgress?: (
      progress: Extract<SessionOutboundMessage, { type: "dissect.progress" }>["payload"],
    ) => void,
    ...
    const unsubscribe = onProgress
      ...
      : null;
    try {
      return await this.sendNamespacedCorrelatedSessionRequest<"dissect.diff.start.response">({
        requestId: resolvedRequestId,
        message: { type: "dissect.diff.start.request", cwd },
        timeout: 900_000,
      });
    } finally {
      unsubscribe?.();
  ...
  async dissectDiffGetFile(
    cwd: string,
    input: { path: string; fromSnapshotId: string; toSnapshotId: string },
    requestId?: string,
  ) {
    return this.sendNamespacedCorrelatedSessionRequest<"dissect.diff.get_file.response">({
      requestId,
      message: {
        type: "dissect.diff.get_file.request",
        cwd,
        path: input.path,
  ...
  async dissectChangesCheck(cwd: string, requestId?: string) {
    return this.sendNamespacedCorrelatedSessionRequest<"dissect.changes.check.response">({
      requestId,
      message: { type: "dissect.changes.check.request", cwd },
      timeout: 120_000,
    });
  }

  async dissectChatAsk(
    cwd: string,
    input: { runId: string; scope: { kind: "folder" | "file"; path: string }; question: string },
    requestId?: string,
  ) {
    return this.sendNamespacedCorrelatedSessionRequest<"dissect.chat.ask.response">({
      requestId,
      message: {
        type: "dissect.chat.ask.request",
        cwd,
        runId: input.runId,
        scope: input.scope,
  ...
  async dissectKnowledgeSignal(
    cwd: string,
    input: { kind: "concept" | "component"; key: string; action: "know" | "explain_more" },
    requestId?: string,
  ) {
    return this.sendNamespacedCorrelatedSessionRequest<"dissect.knowledge.signal.response">({
      requestId,
      message: {
        type: "dissect.knowledge.signal.request",
        cwd,
        target: { kind: input.kind, key: input.key },
        action: input.action,
      },
      timeout: 30_000,
    });

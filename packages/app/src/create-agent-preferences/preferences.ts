...
export function parseFormPreferences(value: unknown): FormPreferences {
  const result = StoredFormPreferencesSchema.safeParse(stripRetiredLaunchTarget(value));
  return result.success ? result.data : DEFAULT_FORM_PREFERENCES;
}

function stripRetiredLaunchTarget(value: unknown): unknown {
  ...
  if (
    ...
    (target as { kind?: unknown }).kind !== "dissect"
  ) {
    return value;
  }
  const { launchTarget: _retired, ...rest } = record;
  return rest;
}

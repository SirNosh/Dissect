...
describe("create agent preferences", () => {
  ...
  it("drops a retired dissect launch target without discarding other preferences", () => {
    expect(parseFormPreferences({ provider: "codex", launchTarget: { kind: "dissect" } })).toEqual({
      provider: "codex",
    });
  });

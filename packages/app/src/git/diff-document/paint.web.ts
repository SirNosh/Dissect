...
export interface PaintWebViewportInput {
  ...
  paintTop?: number;
  paintHeight?: number;
  dissectBlockIndex?: (path: string, cell: DiffCell) => number | null;
}
...
function paintLine(
  ...
  input.row.cells.forEach((cell, cellIndex) => {
    ...
    if (cell) {
      const tint = input.dissectBlockIndex?.(input.row.path, cell);
      if (tint !== null && tint !== undefined) {
        input.context.fillStyle = DISSECT_OVERLAYS[tint % DISSECT_OVERLAYS.length];
        input.context.fillRect(x, input.y, columnWidth, input.row.height);
        input.context.fillStyle = DISSECT_GUTTERS[tint % DISSECT_GUTTERS.length];
        input.context.fillRect(x, input.y, 3, input.row.height);
      }
    }

...
import { matchDissectDiffBlock } from "@/dissect/diff/match-block";
import { positionDissectPopup } from "@/dissect/codemirror/popup-position";
...
export function DiffSurface(props: DiffSurfaceProps) {
  ...
  const [hoveredDissect, setHoveredDissect] = useState<{
    block: DiffBlockDissection;
    left: number;
    top: number;
  } | null>(null);
  const [pinnedDissect, setPinnedDissect] = useState<{
    block: DiffBlockDissection;
    cursor: { x: number; y: number };
  } | null>(null);
  const hasHoveredAffordanceRef = useRef(false);
  const dissectBlocksRef = useRef(props.dissectBlocksByPath);
  dissectBlocksRef.current = props.dissectBlocksByPath;
  ...
  useEffect(() => {
    schedulePaint(true);
  }, [props.dissectBlocksByPath, schedulePaint]);
  useLayoutEffect(() => {
  ...
  const matchDissectFromHit = useCallback((hit: DiffHit | null): DiffBlockDissection | null => {
  ...
  const pointerMove = useCallback(
    (event: React.PointerEvent<HTMLDivElement>) => {
      ...
      const hit = pointHit(event);
      const hover = resolveCellHover({
        hit,
        model: modelRef.current,
        viewportWidth: viewport.width,
        scrollTop: scrollTopRef.current,
        root: rootRef.current,
        pointerXClient: event.clientX,
        dissectBlocks: dissectBlocksRef.current,
      });
      hasHoveredAffordanceRef.current = hover.affordance !== null;
      setHoveredAffordance(hover.affordance);
      setHoveredDissect(hover.dissect);
  ...
  const pointerUp = useCallback(
    (event: React.PointerEvent<HTMLDivElement>) => {
      ...
      const moved = drag
        ...
        : false;
      if (drag && !moved) {
        const block = matchDissectFromHit(hit);
        if (block) {
          setSelection(null);
          setPinnedDissect({ block, cursor: { x: event.clientX, y: event.clientY } });
          return;
        }
        setPinnedDissect(null);
        if (drag.dismissSelectionOnClick) {
          setSelection(null);
          return;
        }
      }
    ...
    [matchDissectFromHit, pointHit, reviewActions, setSelection],
  );
  ...
  const prepareContextMenu = useCallback(
    ...
    [matchDissectFromHit, pointHitAt],
  );
  ...
  const keyDown = useCallback(
    ...
    [pinnedDissect, selectAll, setSelection],
  );
  ...
  const scrollStyle = useMemo<React.CSSProperties>(
    () => ({ ...SCROLL_STYLE, cursor: hoveredDissect ? "pointer" : "text" }),
    [hoveredDissect],
  );
  const dismissPinnedDissect = useCallback(() => {
    setPinnedDissect(null);
  }, []);
  ...
  const surface = (
    <div data-testid="git-diff-canvas-root" ref={rootRef} style={rootStyle}>
      ...
      {pinnedDissect
        ? createPortal(
            <DissectDiffBlockPopup
              block={pinnedDissect.block}
              cursor={pinnedDissect.cursor}
              pane={rootRef.current}
              scroll={scrollRef.current}
              background={props.palette.headerSurface}
              color={props.palette.foreground}
              border={props.palette.border}
              onDismiss={dismissPinnedDissect}
            />,
            getOverlayRoot(),
          )
        : null}
...
function resolveCellHover(input: {
  hit: DiffHit | null;
  model: ReturnType<typeof buildDiffDocumentModel> | null;
  viewportWidth: number;
  scrollTop: number;
  root: HTMLDivElement | null;
  pointerXClient: number;
  dissectBlocks: ReadonlyMap<string, DiffBlockDissection[]> | undefined;
}): {
  affordance: {
    hit: Extract<DiffHit, { kind: "cell" }>;
    left: number;
  ...
  dissect: { block: DiffBlockDissection; left: number; top: number } | null;
} {
  if (input.hit?.kind !== "cell" || !input.model) {
    return { affordance: null, dissect: null };
  }
  const row = input.model.rows[input.hit.position.rowIndex];
  const file = input.model.files[input.hit.position.fileIndex];
  if (!row || !file) return { affordance: null, dissect: null };
  const sideIsNew = input.hit.position.side === "new";
  const splitRow = row.kind === "line" && row.cells.length === 2;
  const sideIndex = splitRow && sideIsNew ? 1 : 0;
  const columnCount = row.kind === "line" ? row.cells.length : 1;
  const gutterBorder = sideIndex * (input.viewportWidth / columnCount) + file.gutterWidth;
  const pointerX = input.root
    ? input.pointerXClient - input.root.getBoundingClientRect().left
    : Number.NaN;
  const showAffordance = Boolean(input.hit.target) && Number.isFinite(pointerX);
  return {
    affordance: showAffordance
      ? {
          hit: input.hit,
          left: gutterBorder - 12,
          top: row.top - input.scrollTop + (input.model.lineHeight - 22) / 2,
        }
      : null,
    dissect: resolveDissectHover({
      row,
      sideIndex,
      blocks: input.dissectBlocks?.get(row.path) ?? [],
      gutterBorder,
...
function resolveDissectHover(input: {
  row: DiffRow;
  sideIndex: number;
  blocks: DiffBlockDissection[];
  gutterBorder: number;
  scrollTop: number;
}): { block: DiffBlockDissection; left: number; top: number } | null {
  if (input.row.kind !== "line") return null;
  ...
  return {
    block: match.block,
    left: input.gutterBorder + 16,
    top: row.top - input.scrollTop + 8,
  };
}
...
const DISSECT_POPUP_BASE: React.CSSProperties = {
  position: "fixed",
...
const DISSECT_POPUP_LABEL: React.CSSProperties = {
  marginTop: 8,
  fontWeight: 600,
  fontSize: 10,
...
function DissectDiffBlockPopup({
  block,
  cursor,
  pane,
  scroll,
  ...
  useLayoutEffect(() => {
    ...
    popup.style.maxWidth = `${Math.min(POPUP_MAX_WIDTH, Math.max(0, paneRect.width - 16))}px`;
    popup.style.maxHeight = `${Math.max(0, paneRect.height - 16)}px`;
    popup.style.width = "";
    ...
    const placed = positionDissectPopup({
      ...
      pane: { x: paneRect.left, y: paneRect.top, width: paneRect.width, height: paneRect.height },
    });

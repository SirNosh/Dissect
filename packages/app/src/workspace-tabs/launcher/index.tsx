...
const BUILT_IN_SELECTIONS = {
  ...
  dissect: { kind: "target", target: { kind: "dissect" } },
...
export function useWorkspaceTabLaunchCatalog(input: {
  ...
  return useMemo(() => {
    ...
    const dissectPresentation = getLaunchPresentation("dissect");
    ...
    const builtIns: Record<BuiltInLaunchItemId, WorkspaceTabLaunchItem & { hidden?: boolean }> = {
      ...
      dissect: {
        id: "dissect",
        label: dissectPresentation.label(t),
        Icon: dissectPresentation.icon,
        disabled: false,
        panelKind: "dissect",
        toggleTarget: BUILT_IN_SELECTIONS.dissect.target,
        launch: launchSelection(BUILT_IN_SELECTIONS.dissect),
      },
      browser: {
        id: "browser",

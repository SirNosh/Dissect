...
import { openWorkspaceDissect } from "@/workspace-tabs/explorer-sidebar";
...
function isNewWorkspacePending(input: {
  pendingAction: "chat" | "empty" | "terminal" | "dissect" | null;
  isDraftHandoffActive: boolean;
}): boolean {
...
const OPEN_DISSECT_ACTIONS: readonly KeyboardActionId[] = ["sidebar.toggle.right"];
const DISSECT_TOGGLE_KEYS = ["mod", "E"];
const COLLAPSED_TOGGLE_ACCESSIBILITY = { expanded: false };

function NewWorkspaceDissectHeaderButton({
  disabled,
  onPress,
}: {
  disabled: boolean;
  ...
  return (
    <WorkspaceExplorerToggle
      onPress={onPress}
      label={t("workspace.tabs.explorerSidebar.open")}
      tooltipLabel={t("workspace.tabs.explorerSidebar.open")}
      tooltipKeys={DISSECT_TOGGLE_KEYS}
      accessibilityState={COLLAPSED_TOGGLE_ACCESSIBILITY}
      mobile={false}
      disabled={disabled}
    />
  );
...
function useNewWorkspaceDissectShortcut(input: {
  isCompact: boolean;
  isPending: boolean;
  hasSelectedSourceDirectory: boolean;
  onOpen: () => void;
}): void {
  const onOpen = input.onOpen;
  useKeyboardActionHandler({
    handlerId: "new-workspace-open-dissect",
    actions: OPEN_DISSECT_ACTIONS,
    enabled: !input.isCompact && !input.isPending && input.hasSelectedSourceDirectory,
    priority: 0,
    handle: () => {
      onOpen();
      return true;
...
function selectLaunchComposer(
  kind: LaunchTarget["kind"],
  composers: { chat: ReactElement; terminal: ReactElement },
): ReactElement {
  return kind === "terminal" ? composers.terminal : composers.chat;
}
...
export function NewWorkspaceScreen({
  ...
  const [pendingAction, setPendingAction] = useState<
    "chat" | "empty" | "terminal" | "dissect" | null
  >(null);
  ...
  const handleOpenDissect = useCallback(async () => {
    try {
      setErrorMessage(null);
      setPendingAction("dissect");
      let outcome: SubmitOutcome = "background";
      await runCreateDissectWorkspace({
        cwd: selectedSourceDirectory ?? "",
        ensureWorkspace: async (request) => (await ensureWorkspace(request)).workspace,
        serverId: selectedServerId,
        navigate: (targetServerId, workspaceId, target) => {
  ...
  const handlePressOpenDissect = useCallback(() => {
    void handleOpenDissect();
  }, [handleOpenDissect]);

  useNewWorkspaceDissectShortcut({
    isCompact,
    isPending,
    hasSelectedSourceDirectory,
    onOpen: handlePressOpenDissect,
  });
  ...
  const screenHeaderRight = useMemo(() => {
    ...
    return (
      <NewWorkspaceDissectHeaderButton
        onPress={handlePressOpenDissect}
        disabled={isPending || !hasSelectedSourceDirectory}
      />
    );
  }, [handlePressOpenDissect, hasSelectedSourceDirectory, isCompact, isPending]);

  const composer = selectLaunchComposer(launchTarget.kind, {
    terminal: (
      <Composer
        key="terminal"
        inputMode="terminal"
